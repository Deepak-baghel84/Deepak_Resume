 ...
 * @param {string} endpoint - The API endpoint (e.g., '/api/classify-image')
 * @returns {string} The full API URL
 */
function getApiUrl(endpoint) {
    // If served from the Python Flask server (port 5000), use relative path
    if (window.location.port === '5000') {
        return endpoint;
    }

    // Otherwise (file://, Live Server, etc.), point to default local backend
...
function classifyImage() {
    ...
    fetch(getApiUrl('/api/classify-image'), {
        method: 'POST',
        headers: {
...
function analyzeSentiment() {
    ...
    if (inputField && resultContainer) {
        ...
        // Call the API endpoint
        fetch(getApiUrl('/api/analyze-sentiment'), {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
